...
pub const AsyncRacingRegistry = struct {
    ...
    pub fn searchRace(self: *Self, query: []const u8) !SearchResult {
        ...
        const SearchTask = struct {
            fn queryRegistry(args: struct {
                ...
                const package = PackageInfo{
                    ...
                    .response_time_ms = @as(u64, @intCast(zion_root.milliTimestamp() - start_time)),
                };
        ...
        const race_start = zion_root.milliTimestamp();
        ...
        while (true) {
            for (results, 0..) |maybe_result, i| {
                if (maybe_result) |result| {
                    ...
                    return SearchResult{
                        ...
                        .response_time_ms = @as(u64, @intCast(zion_root.milliTimestamp() - race_start)),
            ...
            if (zion_root.milliTimestamp() - race_start > 10000) {
    ...
    pub fn benchmarkRegistries(self: *Self) !void {
        ...
        const BenchTask = struct {
            fn benchRegistry(args: struct {
                ...
                args.result_ptr.* = @as(u64, @intCast(zion_root.milliTimestamp() - start_time));
            }
        };
