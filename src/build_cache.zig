...
pub const BuildCache = struct {
    ...
    pub fn computeSourceHash(self: *BuildCache, source_path: []const u8) ![]const u8 {
        ...
        var source_dir = fs.cwd().openDir(source_path, .{ .iterate = true }) catch |err| {
            if (err == error.FileNotFound) {
                // Single file
                const content = try fs.cwd().readFileAlloc(source_path, self.allocator, @enumFromInt(100 * 1024 * 1024));
        ...
        for (file_paths.items) |path| {
            const content = try source_dir.readFileAlloc(path, self.allocator, @enumFromInt(10 * 1024 * 1024));
            defer self.allocator.free(content);
    ...
    pub fn isCacheValid(self: *BuildCache, project_name: []const u8, current_source_hash: []const u8, dependencies: [][]const u8, build_flags: [][]const u8) !bool {
        ...
        for (entry.output_files) |output_file| {
            ...
            fs.cwd().access(full_path, .{}) catch return false;
        }
    ...
    pub fn storeBuildResult(self: *BuildCache, project_name: []const u8, source_hash: []const u8, build_hash: []const u8, dependencies: [][]const u8, build_flags: [][]const u8, output_files: [][]const u8) !void {
        var owned_dependencies: std.ArrayList([]const u8) = .{};
        ...
        var owned_build_flags: std.ArrayList([]const u8) = .{};
        ...
        var owned_output_files: std.ArrayList([]const u8) = .{};
    ...
    pub fn restoreCachedBuild(self: *BuildCache, project_name: []const u8, output_dir: []const u8) !bool {
        ...
        for (entry.output_files) |output_file| {
            ...
            try fs.cwd().copyFile(cached_path, fs.cwd(), target_path, .{});
        }
    ...
    pub fn cacheBuildOutput(self: *BuildCache, _: []const u8, output_files: [][]const u8) !void {
        for (output_files) |output_file| {
            ...
            try fs.cwd().copyFile(output_file, fs.cwd(), cached_path, .{});
        }
    }
    ...
    pub fn clearCache(self: *BuildCache) !void {
        ...
        var cache_dir = fs.cwd().openDir(self.cache_dir, .{ .iterate = true }) catch return;
        ...
        while (try walker.next()) |entry| {
            if (entry.kind == .file) {
                try cache_dir.deleteFile(entry.path);
            }
        }
    ...
    pub fn getStats(self: *const BuildCache, allocator: Allocator) !CacheStats {
        ...
        var cache_dir = fs.cwd().openDir(self.cache_dir, .{ .iterate = true }) catch {
            return CacheStats{
        ...
        while (try walker.next()) |entry| {
            if (entry.kind == .file) {
                const stat = try cache_dir.statFile(entry.path);
