...
pub const RegistryClient = struct {
    ...
    pub fn searchPackages(self: *RegistryClient, query: []const u8, language: ?[]const u8) ![]Package {
        ...
        const search_url = if (std.mem.eql(u8, self.config.name, "github"))
            try std.fmt.allocPrint(self.allocator, "{s}/search/repositories?q={s}+language:{s}", .{ api_url, query, language orelse "zig" })
        else if (std.mem.eql(u8, self.config.name, "zigistry"))
            try std.fmt.allocPrint(self.allocator, "{s}/api/searchPackages?q={s}", .{ api_url, query })
        else
            try std.fmt.allocPrint(self.allocator, "{s}/search?q={s}&language={s}", .{ api_url, query, language orelse "zig" });
        ...
        for (parsed.value.packages) |pkg| {
            ...
            try packages.append(self.allocator, Package{
                ...
                .maintenance_status = if (is_ziglibs)
                    try self.allocator.dupe(u8, "well-maintained")
                else
                    null,
