...
const EcosystemOverviewWidget = struct {
    ...
    fn render(widget: *phantom.widgets.Widget, buffer: *phantom.Terminal.Buffer, area: phantom.Rect) void {
        ...
        // GhostLibs stats
        const gk_stats = std.fmt.allocPrint(self.allocator, "👻 GhostLibs: {} packages across {} categories", .{ self.tui_ref.ghostkellz.packages.items.len, @typeInfo(GhostKellzEcosystem.GhostKellzPackage.Category).@"enum".fields.len }) catch return;
        ...
        // ZigLibs stats
        const zl_stats = std.fmt.allocPrint(self.allocator, "🦎 ZigLibs: {} packages + {} tools", .{ self.tui_ref.ziglibs.packages.items.len, self.tui_ref.ziglibs.tools.items.len }) catch return;
