...
pub fn clean(allocator: Allocator, clean_all: bool) !void {
    ...
    const orphans_removed = cleanupBuildZig(allocator, io, cwd) catch |err| blk: {
    ...
    if (clean_all) {
        ...
        const build_reset = resetBuildZig(allocator, io, cwd) catch |err| blk: {
...
fn cleanupBuildZig(allocator: Allocator, io: Io, cwd: Dir) !bool {
    ...
    const build_content = try readFileContent(allocator, io, cwd, "build.zig");
    defer allocator.free(build_content);

    // Check if there are any zion-added dependencies that no longer exist
    var cleaned_content: std.ArrayList(u8) = .empty;
    ...
    if (found_orphans) {
        ...
        const out_file = try cwd.createFile(io, "build.zig", .{});
        defer out_file.close(io);
        try out_file.writeStreamingAll(io, cleaned_content.items);
...
fn resetBuildZig(allocator: Allocator, io: Io, cwd: Dir) !bool {
    // Check if build.zig exists
    cwd.access(io, "build.zig", .{}) catch |err| {
        if (err == error.FileNotFound) {
            return false;
...
/// Helper function to read entire file content into allocated memory
