...
pub fn hash(allocator: Allocator, args: [][:0]u8) !void {
    if (args.len < 3) {
        printHashHelp();
        return;
    }
...
fn generateHash(allocator: Allocator, args: [][:0]u8) !void {
    if (args.len < 1) {
...
fn verifyHash(allocator: Allocator, args: [][:0]u8) !void {
    if (args.len < 2) {
...
fn updateHash(_: Allocator, args: [][:0]u8) !void {
    if (args.len < 1) {
